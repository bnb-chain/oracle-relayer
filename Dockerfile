...
COPY --from=build /opt/app/build/relayer /opt/app/relayer

ENV RELAYER_HOME /opt/app

ENV BBC_NETWORK 1
ENV CONFIG_FILE_PATH $RELAYER_HOME/config/config.json
...
# Run the app
CMD ./relayer --bbc-network $BBC_NETWORK --config-type $CONFIG_TYPE --config-path $CONFIG_FILE_PATH --aws-region $AWS_REGION --aws-secret-key $AWS_SECRET_KEY